...
### v2.2 (最新版)
- **新增功能**：
  * Base64 文件检测与下载：自动识别请求体中的 base64 数据（支持 Data URL 格式）
  * 支持多种文件格式：PNG、JPG、GIF、PDF、XLSX、DOCX、ZIP 等
  * 智能文件预览：图片和 PDF 直接在新标签页打开，其他文件下载到本地
  * 请求体优化显示：base64 字符串替换为 `[Base64 文件]` 占位符，界面更简洁
  * 控制台日志完整捕获：捕获页面所有 console 输出，包括错误堆栈信息

- **用户界面**：
  * 添加"回到顶部"按钮：请求详情内容较长时，滚动超过 300px 自动显示
  * 状态图标优化：位置下移至 top: 100px，字体大小调整为 16px
  * Base64 文件下载区域：蓝色背景高亮显示，包含文件类型和大小信息
  * 按钮文字智能显示：可预览文件显示"打开"，其他文件显示"下载"

- **功能增强**：
  * 监控窗口后台打开：启用监控时窗口在后台创建，不自动切换焦点
  * 清空历史同步清空控制台：点击"清空历史"按钮同时清除控制台日志
  * Error 对象堆栈捕获：完整记录 JavaScript 错误的堆栈跟踪信息
  * Data URL 格式支持：识别 `data:application/...;base64,...` 格式的文件

- **技术优化**：
  * 控制台拦截优化：将 `addConsoleLog` 函数移至拦截器之前，确保正确执行
  * MIME 类型映射：支持更多文件类型的自动识别（XLSX、DOCX、JSON 等）
  * 文件类型智能判断：通过 MIME 类型或 base64 头部自动识别文件格式
  * 代码结构优化：移除调试代码，提升代码质量
